...
class stepper:
	#instantiate stepper 
	#pins = [stepPin, directionPin, enablePin, limitPin]
	def __init__(self, pins):
		...
		gpio.setup(self.limitPin, gpio.IN)

		#set enable to high (i.e. power is NOT going to the motor)
		gpio.output(self.enablePin, True)
		
		print("Stepper initlimitPinialized (step=" + str(self.stepPin) + ", direction=" + str(self.directionPin) + ", enable=" + str(self.enablePin) + ")")
	...
	def step(self, steps, dir, speed=1, stayOn=False):
		...
		if (dir == 'Up'):
			...
			return False
		gpio.output(self.directionPin, dirDown)

		stepCounter = 0
	
		waitTime = 0.01/speed #waitTime controls speed

		while stepCounter < steps:
			#gracefully exit if ctr-c is pressed
			#exitHandler.exitPoint(True) #exitHandler.exitPoint(True, cleanGPIO)
			#if stepping down stop on limit switch
			limitSw = gpio.input(self.limitPin)
			if (dirDown and not limitSw):
				break
		...
		print("stepperDriver complete (turned " + str(dir) + " " + str(stepCounter) + " steps)")
